...
describe("stores utils", () => {
  describe("getDefaultStoreFormValues()", () => {
    it("storeData가 없으면 기본값을 반환한다", () => {
      expect(getDefaultStoreFormValues(undefined as unknown as never)).toEqual({
        storeCategory: [],
        foodCategory: [],
      });
      expect(getDefaultStoreFormValues({} as unknown as never)).toEqual({
        storeCategory: [],
        foodCategory: [],
      });
      expect(getDefaultStoreFormValues({ success: false })).toEqual({
        storeCategory: [],
        foodCategory: [],
      });
    });
    ...
    it("storeData가 있으면 값을 잘 매핑한다", () => {
      ...
      const result = getDefaultStoreFormValues(storeData);
